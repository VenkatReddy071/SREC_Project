...
const RestaurantOffersDashboard = () => {
    ...
    const fetchDashboardData = useCallback(async () => {
        ...
        setError(null);
        try {
            const offersResponse = await axios.get(`${VITE_SERVER_URL}/api/restaurant/offers/my-restaurant`, {
                headers: { 'Authorization': `Bearer ${MOCK_JWT_TOKEN}` }
            });
            console.log(offersResponse);
            setOffers(offersResponse.data.offers || []);

            const menuItemsResponse = await axios.get(`${VITE_SERVER_URL}/api/restaurant/menu-items/my-restaurant`, {
                headers: { 'Authorization': `Bearer ${MOCK_JWT_TOKEN}` }
            });
    ...
    const addOffer = async (e) => {
        ...
        try {
            const response = await axios.post(`${VITE_SERVER_URL}/api/restaurant/offers/my-restaurant`, offerForm, {
                headers: { 'Authorization': `Bearer ${MOCK_JWT_TOKEN}` }
            });
    ...
    const updateOffer = async (e) => {
        ...
        try {
            await axios.put(`${VITE_SERVER_URL}/api/restaurant/offers/my-restaurant/${editingOffer._id}`, offerForm, {
                headers: { 'Authorization': `Bearer ${MOCK_JWT_TOKEN}` }
            });
    ...
    const deleteOffer = async (id) => {
        if (window.confirm('Are you sure you want to delete this offer?')) {
            try {
                await axios.delete(`${VITE_SERVER_URL}/api/restaurant/offers/my-restaurant/${id}`, {
                    headers: { 'Authorization': `Bearer ${MOCK_JWT_TOKEN}` }
                });
    ...
    const toggleOfferStatus = async (id, currentStatus) => {
        try {
            await axios.put(`${VITE_SERVER_URL}/api/restaurant/offers/my-restaurant/${id}/toggle-status`, { active: !currentStatus }, {
                headers: { 'Authorization': `Bearer ${MOCK_JWT_TOKEN}` }
            });
                                ...
                                <div key={offer._id} className={`p-4 rounded-lg border flex justify-between items-center transition-all duration-200 ${
                                    ...
                                    <div>
                                        ...
                                        <p className={`text-xs mt-1 ${offer.active ? 'text-green-600' : 'text-gray-500'}`}>
                                            ...
                                            {offer.applicable && offer.applicable !== 'all' ? ` | Applies to: ${menuItems.find(item => item._id === offer.applicable)?.name || 'Specific Item'}` : ' | Applies to: All Menu Items'}

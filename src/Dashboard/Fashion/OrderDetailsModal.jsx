...
export const OrderDetailsView = ({
  ...
  const isUserCancelled = selectedOrder.orderStatus === 'user_cancel';
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700 border-b border-gray-200 pb-4 mb-4">
        ...
        <div>
          ...
          <div>
            {selectedOrder?.appliedTaxes?.map((item, index) => (
          ...
          <p>
            ...
            <span className={`font-semibold ${isOrderCompleted ? 'text-green-600' : isUserCancelled || selectedOrder.orderStatus === 'Cancelled' ? 'text-red-600' : 'text-yellow-600'}`}>
          ...
          <ul className="space-y-3">
            {itemsToShow.map((item, index) => (
              <li key={item.product?._id || index} className="flex items-center space-x-3 bg-gray-50 p-3 rounded-md border border-gray-100">
                <img
                  src={item.product?.images?.[0] || "https://placehold.co/50x50/E0E0E0/000000?text=Item"}
      ...
      {isUserCancelled ? (
        <div className="p-6 text-center">
          <p className="text-4xl font-bold text-red-600 mb-2">User Cancelled</p>
          {selectedOrder?.reasion && (
            <p className="text-xl font-semibold text-gray-700">Reason: {selectedOrder.reasion}</p>
          )}
        </div>
      ) : isOrderCompleted ? (
        <div className="p-6 text-center">
          <p className="text-4xl font-bold text-green-600">Order Completed</p>
        </div>
      ) : (
        <div className="p-6">

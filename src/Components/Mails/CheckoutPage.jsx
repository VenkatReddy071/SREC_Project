...
const CheckoutPage = () => {
    ...
    const [selectedPaymentMethod, setSelectedPaymentMethod] = useState('CashOnPickup'); // Default for takeaway
    const [isProcessingOrder, setIsProcessingOrder] = useState(false);
    const [orderConfirmation, setOrderConfirmation] = useState(null);
    const [checkoutError, setCheckoutError] = useState(null);

    const isTakeawayOnly = cart?.items && cart?.items?.every(item => item?.itemModelType === 'Menu');
    const restaurantId=cart?.items[0]?.sourceId;
    // State for offers (you'll fetch these dynamically)
    ...
    if (selectedOffer) {
      ...
      setAdjustedGrandTotal(cartTotals.grandTotal);
    }
    ...
    useEffect(() => {
        ...
        const fetchOffers = async () => {
            try {
                if(isTakeawayOnly){
                    const response=await axios.get(`${import.meta.env.VITE_SERVER_URL}/api/restaurant/offer/${restaurantId}`,{withCredentials:true});
                    console.log(response.data);
                    setMenuOffers(response.data.offers);
                }
    ...
    const handleSelctedOffer = (id) => {
    if (menuOffers) {
        const foundOffer = menuOffers.find((offer) => offer._id?.toString() === id);
    ...
    const cartItems = cart?.items || [];
    const offersToShow = isTakeawayOnly ? menuOffers : productOffers;
                    ...
                    <div>
                        <section className="mb-8 border border-gray-200 p-6 rounded-xl bg-gray-50">
                            ...
                            <div className="border-t border-gray-200 pt-4 mt-4 space-y-2">
                                ...
                                {selectedOffer && ( // Only show "Selected Offer" if an offer is selected
          ...
          <span>
            {selectedOffer.percentage
              ...
              : `-$${selectedOffer.value?.toFixed(2)}`}
                    ...
                    {/* Right Column: Shipping Address & Payment Method (Conditional) */}
                    <div>
                        {/* Shipping Address - Only for Product/Mixed (not takeaway only) */}
                        {!isTakeawayOnly && (
                            <section className="mb-8 border border-gray-200 p-6 rounded-xl bg-gray-50">
                                <h2 className="text-2xl font-bold text-gray-800 mb-4 border-b pb-3 border-gray-200">Shipping Address</h2>
                                <div className="grid grid-cols-1 gap-4">
                        ...
                        {/* Payment Method - Conditional based on isTakeawayOnly */}
                        <section className="border border-gray-200 p-6 rounded-xl bg-gray-50">
                            ...
                            {isTakeawayOnly ? (
                                ...
                                <>
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                        {/* You had Card and UPI as options. I'm putting them back here for non-takeaway */}
                                    ...
                                    {/* Card Details - only shown if selectedPaymentMethod is 'Card' */}
                                    {selectedPaymentMethod === 'Card' && (
                                        <div className="mt-6 p-4 border border-blue-200 bg-blue-50 rounded-lg">
                                            <h3 className="text-lg font-semibold text-blue-800 mb-3">Card Details (Simulated)</h3>
                                            <div className="space-y-3">
                                                <div>
                                                    ...
                                                    <input
                                                        ...
                                                        id="cardNumber"
                                                        name="cardNumber"
                                                        value={customerInfo.cardNumber || ''} // Use customerInfo state for card details now
                                                        onChange={handleCustomerInfoChange} // Update handler
                                                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                                        placeholder="XXXX XXXX XXXX XXXX"
                                                        maxLength="19"
                                                        required={selectedPaymentMethod === 'Card'}
                                    ...
                                    {/* UPI Details - only shown if selectedPaymentMethod is 'UPI' */}
                                    {selectedPaymentMethod === 'UPI' && (
                                        <div className="mt-6 p-4 border border-purple-200 bg-purple-50 rounded-lg">
                                            <h3 className="text-lg font-semibold text-purple-800 mb-3">UPI Details (Simulated)</h3>
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    id="upiId"
                                                    name="upiId"
                                                    value={customerInfo.upiId || ''} // Use customerInfo state for UPI ID
                                                    onChange={handleCustomerInfoChange} // Update handler
                                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                                                    placeholder="yourname@bankupi"
                                                    required={selectedPaymentMethod === 'UPI'}
                        ...
                        {/* Offers Section */}
                        <section className="mt-8 border border-gray-200 p-6 rounded-xl bg-gray-50">
                            <h2 className="text-2xl font-bold text-gray-800 mb-4 border-b pb-3 border-gray-200">Available Offers</h2>
                            {offersToShow?.length === 0 ? (
                                <p className="text-gray-600">No specific offers available for {isTakeawayOnly ? 'Menu items' : 'Products'} at this time.</p>
                            ) : (
                                <div className="space-y-3">
                                    {offersToShow?.map(offer => (
                                        <label key={offer._id} className="flex items-center space-x-3 p-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors duration-200">
                                            <input
                                                type="radio"
                                                name="selectedOffer"
                                                value={offer._id}
                                                checked={selectedOffer?._id === offer._id}
                                                onChange={(e) => handleSelctedOffer(e.target.value)}
                                                className="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500"
                                            />
                                            <span className="text-gray-800 font-medium">{offer.name}</span>
                                            {offer.discount > 0 && <span className="text-green-600 text-sm">({(offer.discount * 100)}% off)</span>}
                                        </label>
                                    ))}
                                </div>
                            )}
                    ...
                    <button
                        ...
                        disabled={
                            ...
                            (!isTakeawayOnly && (
                                ...
                                !customerInfo.shippingAddress.country ||
                                !selectedPaymentMethod ||
                                (selectedPaymentMethod === 'Card' && (!customerInfo.cardNumber || !customerInfo.expiryDate || !customerInfo.cvv)) ||
                                (selectedPaymentMethod === 'UPI' && !customerInfo.upiId)
                            ))
                        }
                    >
                        {isProcessingOrder ? 'Processing Order...' : 'Place Order'}

...
export const HospitalsSection = ({ id, activateAmbulanceFilter }) => {
  ...
  const fetchHospitals = async (reset = false) => {
    ...
    const filterParams = {
      ...
      ...(selectedSpecialties.size > 0 && { specialization: Array.from(selectedSpecialties).join(',') }),
  ...
  useEffect(() => {
    if (activateAmbulanceFilter) {
      setAmbulanceFilter(true);
      setShowFilterSidebar(true);
      handleApplyFilters();
    }
  }, [activateAmbulanceFilter]);
                      ...
                      >
                        {item?.ambulance && (
                          ...
                          )}
                          {
                            item?.isOffer 
                            ?(
                              <span className="absolute top-2 left-2 bg-blue-500 text-white text-xs font-bold px-2 py-1 rounded-full z-6 shadow-sm font-['Inter']">
                                24/7 Facility
                            </span>
                            )
                            :

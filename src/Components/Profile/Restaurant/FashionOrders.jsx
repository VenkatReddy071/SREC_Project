...
const MyMallOrders = () => {
    ...
    const fetchOrders = useCallback(async (page) => {
        ...
        try {
            const response = await Axios.get(
                `${SERVER_URL}/api/order/orders/user?sourceType=Mall&page=${page}&limit=${ITEMS_PER_LOAD}`,
                {
    ...
    return (
        <div className="w-full mx-auto px-4 bg-gray-50 min-h-screen py-8">
            <h1 className="text-3xl font-extrabold text-gray-900 mb-8 text-center border-b pb-4">Your Mall Orders</h1>
                ...
                <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-2 gap-6">
                        {orders.map((order, index) => {
                            ...
                            const orderItemImage = order.items?.[0]?.product?.images?.[0] || "https://placehold.co/150x150/f0f9ff/00796b?text=Order+Image";
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700 text-sm mb-6">
                            <div>
                                <p className="font-semibold text-gray-600 flex items-center">
                                    <FaStore className="mr-2 text-blue-500 w-4 h-4" /> Mall:
                                </p>
                                <p className="ml-5">
                                    <b>Name: </b>
                            ...
                            <div className="space-y-4 text-gray-700 text-sm">
                                {selectedOrder.items && selectedOrder.items.length > 0 ? (
                                    selectedOrder.items.map((item, idx) => (
                                        <div key={item._id || idx} className="flex flex-col sm:flex-row items-start sm:items-center bg-gray-50 p-3 rounded-md shadow-sm">
                                            <div className="flex-shrink-0 mr-4 mb-2 sm:mb-0">
                                                <img
                                                    src={item.product?.images?.[0] || "https://placehold.co/80x80/f0f9ff/00796b?text=Item"}
                                                    alt={item.name}
                                            ...
                                            <div className="flex-grow">
                                                <p className="font-medium text-base mb-1">{item.quantity} x {item.name}</p>
                                                <p className="text-gray-600 text-xs">
                                                    Price: {item.price.toFixed(2)} {item.currency}
                                                </p>
                                                {item.product?.category && (
                                                    <p className="text-gray-600 text-xs">Category: {item.product.category}</p>
                                                )}
                                                {item.product?.brand && (
                                                    <p className="text-gray-600 text-xs">Brand: {item.product.brand}</p>
                                                )}
                                                {item.product?.gender && (
                                                    <p className="text-gray-600 text-xs">Gender: {item.product.gender}</p>
                                                )}
                                                {item.selectedSize && (
                                                    <p className="text-gray-600 text-xs">Size: {item.selectedSize}</p>
                                                )}
                                                {item.selectedColor && (
                                                    <p className="text-gray-600 text-xs">Color: {item.selectedColor}</p>
                                                )}
                                                {item.storeName && (
                                                    <p className="text-gray-600 text-xs">Store: {item.storeName}</p>
                                                )}
                                            </div>
                                        </div>
                                    ))
            ...
            >
                <div className="p-6 bg-white rounded-lg relative max-w-lg w-full max-h-[90vh] overflow-y-auto">
                    ...
                    <textarea
                        ...
                        placeholder="e.g., changed my mind, ordered by mistake, store closed"
                        value={cancelReason}

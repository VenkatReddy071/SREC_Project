        ...
        <div className="container mx-auto bg-white shadow-lg rounded-xl overflow-hidden my-8 p-4 sm:p-6 lg:p-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                <div>
                    <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-2">{restaurant.name}</h1>
                    <div className="flex items-center mb-4">
                        {renderStars(restaurant.rating, restaurant.reviews)}
                        {restaurant.isTopPick && (
                            <span className="ml-4 bg-yellow-100 text-yellow-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">
                                Top Pick
                            </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-gray-50 p-6 rounded-lg shadow-sm flex flex-col">
                    ...
                    {(restaurant.latitude !== undefined && restaurant.latitude !== null && restaurant.longitude !== undefined && restaurant.longitude !== null) ? (
                        <div>
                            <h3 className="text-xl font-semibold text-gray-800 mb-4 mt-6">Location Map</h3>
                            <div className="w-full h-64 rounded-lg overflow-hidden border border-blue-100 z-10">
                                <MapContainer
                                    center={[restaurant.latitude, restaurant.longitude]}
                                    zoom={15}
                        ...
                        <div className="w-full h-64 bg-gray-200 rounded-lg flex items-center justify-center text-gray-500 text-center mt-6">
                            Map coordinates not available.
                        </div>
                    )}
                </div>
                    ...
                    <div className="bg-gray-50 p-6 rounded-lg shadow-sm">
                        ...
                        <ul className="list-none p-0 space-y-1">
                            {restaurant.operatingHours && restaurant.operatingHours.length > 0 ? (
                                restaurant.operatingHours.map((dayHours, index) => (
                            ...
                            <div className="mt-6">
                                ...
                                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-4 text-gray-600">
                                    {restaurant.services.map((service) => (
                                        ...
                                        <li key={service} className="flex items-center">
                                            {serviceIcons[service] || <span className="mr-2 text-gray-500">&#8226;</span>}
                                            {service}
                                        </li>
                                    ))}
                                </ul>
            ...
            {restaurant.imageUrls?.galleryImages && restaurant.imageUrls.galleryImages.length > 0 && (
                <section className="mb-8 p-6 bg-blue-50 rounded-lg border border-blue-200">
                    <h3 className="text-2xl font-bold text-blue-700 mb-6">Restaurant Gallery</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {restaurant.imageUrls.galleryImages.map((imgUrl, index) => (
                            <GalleryImage key={index} imgUrl={imgUrl} index={index} openImageModal={openImageModal} />
                        ))}
                    </div>
                </section>
            )}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
                    <div className="relative bg-white rounded-lg p-2 max-w-full max-h-full overflow-auto">
                        ...
                        <img
                            src={currentImage}
                            alt="Full Screen Menu"
                            className="max-w-[90vw] max-h-[90vh] object-contain z-50"
                            onError={(e) => {
                                e.target.onerror = null;
                                e.target.src = `https://placehold.co/800x600/E0E0E0/333333?text=Image+Load+Failed`;
                            }}
...
const GalleryImage = ({ imgUrl, index, openImageModal }) => {
    const [imageLoaded, setImageLoaded] = useState(false);

    return (
        <div
            className="relative w-full h-48 rounded-lg overflow-hidden border border-blue-100 cursor-pointer"
...
// Renamed renderStars to be a separate function if not already globally defined
const renderStars = (rating, reviews) => {
    ...
    return (
        <div className="flex items-center">
            ...
            {reviews !== undefined && <span className="ml-2 text-gray-600 text-sm">({reviews} reviews)</span>}
        </div>
    );
};

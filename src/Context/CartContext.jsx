...
export const CartProvider = ({ children }) => {
    ...
    const fetchCart = useCallback(async () => {
        ...
        try {
            ...
            setCartLength(response?.data?.cart?.items?.length || 0);
    ...
    const addToCart = useCallback(async (product) => {
        ...
        setError(null);
        try {
            const response = await axios.post(`${VITE_SERVER_URL}/api/cart/add`, {
                product
            }, { withCredentials: true });
            await fetchCart();
    ...
    const updateCartQuantity = useCallback(async ({ itemId, quantity, itemModelType, sourceType, sourceId, selectedSize, selectedColor }) => {
        setLoading(true);
        setError(null);
        console.log(itemId, quantity, itemModelType, sourceType, sourceId, selectedSize, selectedColor );
        try {
            const response = await axios.put(`${VITE_SERVER_URL}/api/cart/update-quantity`, {
    ...
    const removeFromCart = useCallback(async ({ itemId, itemModelType, sourceType, sourceId, selectedSize, selectedColor }) => {
        setLoading(true);
        setError(null);
        try {
            const response = await axios.delete(`${VITE_SERVER_URL}/api/cart/remove`, {
                data: {
                    itemId,
                    itemModelType,
                    sourceType,
    ...
    const clearCart = useCallback(async () => {
        ...
        setError(null);
        try {
            await axios.delete(`${VITE_SERVER_URL}/api/cart/clear`, { withCredentials: true });
            await fetchCart();
            showCartMessage('Cart cleared.', 'success');
        } catch (err) {
            console.error("Error clearing cart:", err);
